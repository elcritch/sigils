...
suite "JSON-RPC selector helper thread":
  test "returns protocol dispatch to the application thread":
    ...
    block ioLifetime:
      ...
      try:
        ...
        socket.send(
          """{"jsonrpc":"2.0","method":"calculator.addNumbers","params":[5,6],"id":"helper"}
"""
        )
        discard home.poll()
        let reply = parseJson(socket.recvLine(timeout = 2_000))
        check reply["result"].getInt() == 11
        check reply["id"].getStr() == "helper"
      finally:
        socket.close()
