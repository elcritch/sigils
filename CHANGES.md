...
- `v0.28.0` - run Atlas-based CI on Linux with GCC and Clang, macOS with Clang, and Windows with GCC, with Chronos tests enabled on every job; measure idle CPU usage across the whole process and make pool concurrency checks independent of OS scheduling; keep endpoint diagnostic printing portable across native lock implementations
